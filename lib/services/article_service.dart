  ...
  Future<List<Article>> _fetchFromRSS(String feedUrl, String sourceName) async {
    try {
      ...
      return items.map((item) {
        final title = _cleanText(item.findElements('title').single.text);
        final description = _cleanText(item.findElements('description').single.text);
        final url = item.findElements('link').single.text;
        final publishedAt = item.findElements('pubDate').single.text;
